...
data isn't ready for AI, we say so openly and show what needs fixing
first.

**3. Proposal & agreement.** Scope, phases, timeline, price and
**acceptance criteria written as numbers** (e.g. minimum accuracy,
response time). The agreement is signed after approval.
...
existing systems, and an interface is built where needed. Weekly status +
demo.

**7. Testing & validation.** In real scenarios, with the client's team
involved (UAT). The acceptance criteria in the agreement are checked one
by one.

**8. Go-live.** Production deployment + monitoring (performance, drift,
errors). A rollback plan is ready in advance.
